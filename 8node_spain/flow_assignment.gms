...
def_bud.. budvar =e= sum(i, station_capacity_slope(i)*(s_prev(i)+sh_prev(i)))
         + sum(i$(s_prev(i) >= 0.05),
                        station_cost(i))
         + lam*sum(i$(sh_prev(i) >= 0.05),
                        hub_cost(i));

* ---------- Objetivo ----------
def_op_obj..
    op_obj =e=  sum((i,j), op_link_cost(i,j) * a_prev(i,j));
...
link_cap(i,j)..
     sum((o,d), zij(i,j,o,d) * demand(o,d)) =l= a_prev(i,j) * a_nom * tau;

station_cap(i)..   sigma * (sum(j, a_prev(j,i)) + sum(j, a_prev(i,j))) =l= s_prev(i) + sh_prev(i) + epsi;

hub_connect_cap(i).. sigma * (sum( (o,d,j)$(ord(o)<>ord(i)),
    demand(o,d) * zij(i,j,o,d) / (a_nom * tau))  + sum( (o,d,j)$(ord(d)<>ord(i)),
    demand(o,d) * zij(j,i,o,d) / (a_nom * tau))) =l= sh_prev(i);
    
hub_direct_cap(o,d).. sigma * demand(o,d) * zij(o,d,o,d) / (a_nom * tau) =l= sh_prev(o) + sh_prev(d);

tot_links.. sum((i,j), a_prev(i,j)) =l= a_max + epsi;
