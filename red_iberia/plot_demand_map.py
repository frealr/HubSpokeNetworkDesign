...
def main():
    ...
    # ── Load demand data (todos los mercados con demanda > 0) ────────────────
    df = pd.read_excel(xlsx_path, sheet_name='demanda_completa')
    df = df.dropna(subset=['Origen', 'Destino'])
    iata_re = re.compile(r'^[A-Z]{3}$')
    df = df[df['Origen'].apply(lambda x: bool(iata_re.match(str(x)))) &
