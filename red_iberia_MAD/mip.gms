...
link_cap(i,j)..
     sum((o,d), zij(i,j,o,d) * demand(o,d)) =l= a(i,j) * a_nom(i,j) * tau;

station_cap(i)..   sigma * (sum(j, a(j,i)) + sum(j, a(i,j))) =l= s(i) + sh(i);

hub_connect_cap(i).. sigma * (sum( (o,d,j)$(ord(o)<>ord(i)),
    demand(o,d) * zij(i,j,o,d) / (a_nom(i,j) * tau))  + sum( (o,d,j)$(ord(d)<>ord(i)),
    demand(o,d) * zij(j,i,o,d) / (a_nom(j,i) * tau))) =l= sh(i);

hub_direct_cap(o,d).. sigma * demand(o,d) * zij(o,d,o,d) / (a_nom(o,d) * tau) =l= sh(o) + sh(d);
