...
def build_network():
    ...
    Solo incluye los mercados con yield directo real (no vía MAD ni media global).
    Esto corresponde a los pares OD presentes en yield.xlsx.
    """
    iata_re = re.compile(r'^[A-Z]{3}$')
    ...
    # ── Demanda: solo mercados con yield directo Y ambos extremos permitidos ──
    ...
    dd_full = dd_full[dd_full['Origen'].apply(lambda x: bool(iata_re.match(str(x)))) &
                      dd_full['Destino'].apply(lambda x: bool(iata_re.match(str(x))))]
    dd_full = dd_full[dd_full['Promedio de Suma de Demanda'] > 0]

    dd = dd_full[dd_full.apply(
        lambda row: (
            row['Origen'] in allowed_nodes and
            row['Destino'] in allowed_nodes and
            get_yield(row['Origen'], row['Destino'])[1] == 'directo'
        ), axis=1
    )].copy()
    ...
    for _, row in dd.iterrows():
        ...
        counts[cat] += 1
    print(f'  Nodos: {n}  |  Mercados (yield directo, ambos en airports.csv): {len(dd)}')
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            # Evita underflow en exp(.) cuando los precios hacen la utilidad muy negativa.
...
if __name__ == '__main__':
    ...
    print('Construyendo red iberia MAD (solo mercados con yield directo)…')
