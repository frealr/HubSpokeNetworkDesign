...
The formal justification rests on the convex-conjugate duality between the log-sum-exp aggregator and the negative entropy \cite{Oppenheim1993,RealRojas2026TRB}: the unique minimizer of the combined utility--entropy objective over $\Delta^{od}$ recovers exactly the logit probabilities \eqref{eq:logit_probability}. Consequently, market shares are not imposed externally but arise endogenously from the lower-level optimality conditions, while the model remains smooth, convex, and computationally tractable.
